...
window.addEventListener('DOMContentLoaded', function () {
  'use strict'; // сохранение выбранных параметров рассчета калькулятора
        ...
        switch (item.nodeName) {
          ...
          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              i === 0 ? state[prop] = 'Холодное' : state[prop] = 'Теплое'; // выбор отмеченного чекбокса пользователем
...
var forms = function forms(state) {
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            document.querySelector('.status').textContent = message.loading; //5 настройка запроса
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); //1 создаем блок с сообщением

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      item.appendChild(statusMessage); //2 собираем все данные с формы

      var formData = new FormData(item); // 2.1 отправка данных расчета замеров
      ...
      postData('assets/server.php', formData).then(function (res) {
        console.log(res); //7 уведомление пользователя
...
var images = function images() {
  ...
  workSection.appendChild(imgPopup); // стили для модального окна
...
var modals = function modals() {
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        modal.style.display = 'block'; // отмена прокрутки под модальным окном
  ...
  } // ф-я фиксирования window при вызове модального окна
...
var timer = function timer(id, deadline) {
  // ф-я получения оставшегося времени до конца акции
